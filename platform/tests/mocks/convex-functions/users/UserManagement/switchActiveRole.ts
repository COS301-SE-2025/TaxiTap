...
export const switchActiveRoleHandler = jest.fn().mockImplementation(
  async (ctx: any, args: { 
    ...
    if (args.newRole === "passenger") {
      ...
      const activeDriverRides = await ctx.db
        .query("rides")
        .withIndex("by_driver", (q: any) => q.eq("driverId", args.userId))
        .filter((q: any) => 
          q.or(
      ...
      const activePassengerRides = await ctx.db
        .query("rides")
        .withIndex("by_passenger", (q: any) => q.eq("passengerId", args.userId))
        .filter((q: any) => 
          q.or(
