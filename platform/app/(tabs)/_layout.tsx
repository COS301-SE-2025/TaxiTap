...
export default function HomeScreen() {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({ 
      ...
      // Prevent header from being transparent and overlapping content
  ...
  const dynamicStyles = StyleSheet.create({
    ...
    map: { 
      // iOS-specific height adjustments to prevent tab header overlap
      height: Platform.OS === 'ios' 
        ? (keyboardVisible ? '28%' : '37%')  // Slightly reduced for iOS
        : (keyboardVisible ? '30%' : '40%'),  // Android unchanged
    },
    bottomSheet: {
      flex: 1,
      ...
      padding: 24,
      paddingTop: 32,
      paddingBottom: keyboardVisible ? Math.max(keyboardHeight - 100, 24) : Math.max(insets.bottom, 24),
      borderWidth: 1,
      borderColor: isDark 
        ? 'rgba(71, 85, 105, 0.3)' 
    ...
    reserveButton: {
      position: 'absolute',
      bottom: Math.max(insets.bottom + 20, 80), // Adjust for safe area on iOS
      left: 24,
      right: 24,
